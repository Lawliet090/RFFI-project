...
globals [ correlation-matrix company-names max-size min-size turtles-created ]
...
to setup
  ...
  foreach range num-agents [ i ->
    ...
    set-current-plot "Battery Levels"
    clear-plot
...
;; ============================
;; Show Battery Levels (Monitor)
;; ============================
to-report show-battery-levels
  let newbatterylevel []

  ask turtles [
    set newbatterylevel lput (word agent-name ": " precision battery-level 2) newbatterylevel
  ]

  let battery-level-string ""
  foreach newbatterylevel [ battery-item ->
    set battery-level-string (word battery-level-string battery-item ", ")
  ]

  if length battery-level-string > 0 [
    let battery-level-length (length battery-level-string)
    set battery-level-string (substring battery-level-string 0 (battery-level-length - 3))
  ]

  report battery-level-string
end
...
to go
  ...
  ;; Update each agent's battery logic and calculate surplus
  ask turtles [
    ;; Store the previous battery level
    set previous-battery-level battery-level


    ;; Update battery-level by adding surplus
    set battery-level battery-level + surplus-energy

    ;; Calculate surplus A = P - C
    let a energy-production - energy-consumption

    if a > 0 [
      set battery-level battery-level + a
      show (word "Agent " agent-name " has surplus " precision a 2 " → battery now " precision battery-level 2)
    ]

    if a < 0 [
      let needed-energy abs a
      let energy-drawn min list needed-energy battery-level
      set battery-level battery-level - energy-drawn
      set a a + energy-drawn

      show (word "Agent " agent-name ": Deficit " precision needed-energy 2
        ", Drew " precision energy-drawn 2
        ", Battery now " precision battery-level 2
        ", A now " precision a 2)

      if a < 0 [
        show (word "Agent " agent-name " still has unmet deficit of " precision abs a 2)
      ]
    ]
  ]
  ...
  set-current-plot "Battery Levels"
  foreach company-names [ name ->
    let consumer one-of turtles with [agent-name = name and shape = "square"]
    let producer one-of turtles with [agent-name = name and shape = "circle"]

    if consumer != nobody and producer != nobody [
      set-current-plot-pen name
      let a ([energy-production] of producer) - ([energy-consumption] of consumer)
      plot a
    ]
  ]
  ...
  ask turtles [
    ...
    ;; Update consumption and production depending on shape
    if shape = "square" [  ;; Consumers
  ...
  ;;  Update link thickness and color based on correlation
  ask links [
    let index1 position [agent-name] of end1 company-names
    let index2 position [agent-name] of end2 company-names
    if index1 != false and index2 != false [
...
"" ""
PENS
"Salsus AS.xlsx" 1.0 0 -723837 true "" ""
"Ocka Eiendom AS.xlsx" 1.0 0 -13791810 true "" ""
"Stantek AS.xlsx" 1.0 0 -2674135 true "" ""
"Schutz Nordic AS.xlsx" 1.0 0 -955883 true "" ""
"Maarud AS.xlsx" 1.0 0 -6459832 true "" ""
"Norske Backer AS.xlsx" 1.0 0 -1184463 true "" ""
"Instruct AS (Kundenavn Enger Kasio AS).xlsx" 1.0 0 -4699768 true "" ""
"Hydro Extrusion Norway AS.xlsx" 1.0 0 -1264960 true "" ""
"Home Brands AS.xlsx" 1.0 0 -14835848 true "" ""
"First Seafood AS.xlsx" 1.0 0 -11221820 true "" ""
"Blowtech AS.xlsx" 1.0 0 -11783835 true "" ""
"Berggren eiendom AS.xlsx" 1.0 0 -13345367 true "" ""
